...
class ux_t3lib_extFileFunctions extends t3lib_extFileFunctions {
		...
		foreach ($searchFieldsArray as $table => $fields) {
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				...
				foreach ($fields as $field) {
					...
					if ($conf['type'] == 'flex') {
						if(strpos($row[$field], $theTarget) !== false) {
							$results[$table][$row['uid']][$field] = $this->replaceData($row[$field], $theTarget, $theRenameName, $field);
						}
					}
				}
	...
	function replaceData($data, $theTarget, $theRenameName, $field) {
		$result = '';
		switch($field) {
			case 'bodytext':
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/t3filelist/class.ux_t3lib_extfilefunc.php'])	{
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/t3filelist/class.ux_t3lib_extfilefunc.php']);
}
?>
